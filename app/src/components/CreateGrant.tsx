...
export default function CreateGrant({ setpreview }) {
  ...
  const handleSubmit = async () => {
    ...
    if (
      new Date(grant.dueDate + " 00:00:00").getTime() <= new Date().getTime()
    ) {
      return notify({
        type: "error",
        message: "error",
        description: "Due date entered must be in the future!",
      });
    }
    ...
    const uploadResult = await uploadToWeb3DB(
      wallet,
      grant,
      setTransactionsList
    );
  ...
  return (
    <>
      <Modal
        ...
        showCloseButton={!showTransactionFlow}
                  ...
                  <label>
                    <div className="grantsub">
                      ...
                      <input
                        className="grantinput"
                        type="number"
                        placeholder="Grant Target Amount"
                        name="targetAmount"
                        value={grant.targetAmount}
                        onChange={handleChange}
                      />
                    </div>
                  </label>
