...
pub fn create_donation(ctx: Context<CreateDonation>, lamports: u64) -> Result<()> {
    ...
    ctx.accounts
        .donation.set_inner(Donation::new(
            *ctx.bumps.get(Donation::SEED_PREFIX).expect("we should have gotten the donation canonical bump"),
            ctx.accounts.payer.key(), 
            ctx.accounts.grant.key(), 
            lamports 
        ));
    ...
    ctx.accounts
        .grant
        .update_with_donation(&ctx.accounts.donation.clone().into_inner());
